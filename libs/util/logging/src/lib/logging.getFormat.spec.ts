...
describe("getLoggerFormat", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(format, "colorize")
      .mockReturnValue(FORMAT_COLORIZE as unknown as Colorizer);
    jest
      .spyOn(format, "errors")
      .mockReturnValue(FORMAT_ERRORS as unknown as Format);
    jest
      .spyOn(format, "simple")
      .mockReturnValue(FORMAT_SIMPLE as unknown as Format);
  ...
  it("returns a combined format with errors, timestamp, and colorize when not in production", () => {
